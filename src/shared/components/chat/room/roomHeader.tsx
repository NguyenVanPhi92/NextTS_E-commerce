...
export const RoomHeader = ({ data, onClick, showBackBtn }: RoomHeaderProps) => {
  const dispatch = useDispatch()
  return (
    <div className="h-full px-16 flex-center">
      <div className={`mr-12 ${showBackBtn ? "" : "md:hidden"}`}>
          ...
          <div className="flex items-center">
            {data.room_type === "group" ? (
              <button onClick={onClick} className="flex items-center cursor-pointer">
                <HiOutlineUsers className="hidden sm:block mr-4 text-base w-[16px] h-[16px]" />
