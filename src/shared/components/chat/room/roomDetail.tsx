...
import { useDetectWindowFocus, useMessage, useRoomDetail } from "shared/hook"
...
interface RoomDetailProps {
  ...
  showBackBtnOnHeader?: boolean
}

export const RoomDetail = forwardRef(function RoomChild(
  { onSendMessage, showBackBtnOnHeader }: RoomDetailProps,
  ...
  return (
    <div className="flex flex-col flex-1 bg-white-color">
      {isFirstLoading ? (
        <Spinner />
      ) : (
        <>
          <div className="h-[70px] border-b border-border-color border-solid">
            <RoomHeader
              showBackBtn={showBackBtnOnHeader}
              onClick={() => {
                if (data?.room_id) {
