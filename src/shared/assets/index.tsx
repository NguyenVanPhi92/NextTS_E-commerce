export const paymentImg = require("./images/payments.jpg");
export const popupImg = require("./images/popup.png");
export const avatar = require("./images/avatar.png");
export const emptyImg = require("./images/empty.png");
export const satavanRoadMap = require("./images/satavan-develop-roadmap.png");
export const imageBlur =
	"data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVR42mN8/+F9PQAI8wNPvd7POQAAAABJRU5ErkJggg==";
export const filterNotFound = require("./images/filter-not-found.png");
export const loginBg = require("./images/login_bg.jpg");
export const bannerLeft = require("./images/banner-left.jpg");
export const qrCodeImg = require("./images/qr-code.jpg");
export const shopBannerImg = require("./images/shop-banner.jpg");
export * from "./icons";
export const cartEmptyIcon = (
	<svg
		xmlns="http://www.w3.org/2000/svg"
		viewBox="0 0 280.028 280.028"
		width="280.028"
		height="80.028"
	>
		<path className="c-01" d="M35.004 0h210.02v78.758H35.004V0z" fill="#d07c40"></path>
		<path
			className="c-02"
			d="M262.527 61.256v201.27c0 9.626-7.876 17.502-17.502 17.502H35.004c-9.626 0-17.502-7.876-17.502-17.502V61.256h245.025z"
			fill="#f4b459"
		></path>
		<path
			className="c-03"
			d="M35.004 70.007h26.253V26.253L35.004 0v70.007zm183.767-43.754v43.754h26.253V0l-26.253 26.253z"
			fill="#f4b459"
		></path>
		<path
			className="c-04"
			d="M61.257 61.256V26.253L17.503 61.256h43.754zm157.514-35.003v35.003h43.754l-43.754-35.003z"
			fill="#e3911c"
		></path>
		<path
			className="c-05"
			d="M65.632 105.01c-5.251 0-8.751 3.5-8.751 8.751s3.5 8.751 8.751 8.751 8.751-3.5 8.751-8.751c0-5.25-3.5-8.751-8.751-8.751zm148.764 0c-5.251 0-8.751 3.5-8.751 8.751s3.5 8.751 8.751 8.751 8.751-3.5 8.751-8.751c.001-5.25-3.501-8.751-8.751-8.751z"
			fill="#cf984a"
		></path>
		<path
			className="c-06"
			d="M65.632 121.637c5.251 0 6.126 6.126 6.126 6.126 0 39.379 29.753 70.882 68.257 70.882s68.257-31.503 68.257-70.882c0 0 .875-6.126 6.126-6.126s6.126 6.126 6.126 6.126c0 46.38-35.003 83.133-80.508 83.133s-80.508-37.629-80.508-83.133c-.001-.001.874-6.126 6.124-6.126z"
			fill="#cf984a"
		></path>
		<path
			className="c-07"
			d="M65.632 112.886c5.251 0 6.126 6.126 6.126 6.126 0 39.379 29.753 70.882 68.257 70.882s68.257-31.503 68.257-70.882c0 0 .875-6.126 6.126-6.126s6.126 6.126 6.126 6.126c0 46.38-35.003 83.133-80.508 83.133s-80.508-37.629-80.508-83.133c-.001 0 .874-6.126 6.124-6.126z"
			fill="#fdfbf7"
		></path>
	</svg>
);
export const view4Icon = (
	<svg width="14" height="14" viewBox="0 0 14 14" fill="none" xmlns="http://www.w3.org/2000/svg">
		<path d="M0 0H2V2H0V0Z"></path>
		<path d="M0 4H2V6H0V4Z"></path>
		<path d="M0 8H2V10H0V8Z"></path>
		<path d="M0 12H2V14H0V12Z"></path>
		<path d="M4 0H6V2H4V0Z"></path>
		<path d="M4 4H6V6H4V4Z"></path>
		<path d="M4 8H6V10H4V8Z"></path>
		<path d="M4 12H6V14H4V12Z"></path>
		<path d="M8 0H10V2H8V0Z"></path>
		<path d="M8 4H10V6H8V4Z"></path>
		<path d="M8 8H10V10H8V8Z"></path>
		<path d="M8 12H10V14H8V12Z"></path>
		<path d="M12 0H14V2H12V0Z"></path>
		<path d="M12 4H14V6H12V4Z"></path>
		<path d="M12 8H14V10H12V8Z"></path>
		<path d="M12 12H14V14H12V12Z"></path>
	</svg>
);
export const view3Icon = (
	<svg width="15" height="15" viewBox="0 0 15 15" fill="none" xmlns="http://www.w3.org/2000/svg">
		<path d="M0 0H3V3H0V0Z"></path>
		<path d="M0 6H3V9H0V6Z"></path>
		<path d="M0 12H3V15H0V12Z"></path>
		<path d="M6 0H9V3H6V0Z"></path>
		<path d="M6 6H9V9H6V6Z"></path>
		<path d="M6 12H9V15H6V12Z"></path>
		<path d="M12 0H15V3H12V0Z"></path>
		<path d="M12 6H15V9H12V6Z"></path>
		<path d="M12 12H15V15H12V12Z"></path>
	</svg>
);
export const view2Icon = (
	<svg width="15" height="15" viewBox="0 0 15 15" fill="none" xmlns="http://www.w3.org/2000/svg">
		<path d="M0 0H6V6H0V0Z"></path>
		<path d="M0 9H6V15H0V9Z"></path>
		<path d="M9 0H15V6H9V0Z"></path>
		<path d="M9 9H15V15H9V9Z"></path>
	</svg>
);
export const view1Icon = (
	<svg width="15" height="15" viewBox="0 0 15 15" fill="none" xmlns="http://www.w3.org/2000/svg">
		<path d="M0 0H15V3H0V0Z"></path>
		<path d="M0 6H15V9H0V6Z"></path>
		<path d="M0 12H15V15H0V12Z"></path>
	</svg>
);
export const pageNotFound = require("./images/page-not-found.png");
export const messageIcon = (
	<svg width="18" height="18" viewBox="0 0 18 18" xmlns="http://www.w3.org/2000/svg">
		<path
			fillRule="evenodd"
			clipRule="evenodd"
			d="M0 8.25C0 3.95165 4.15905 0.75 9 0.75C13.8409 0.75 18 3.95165 18 8.25C18 10.0141 17.2499 11.5969 16.0855 12.8642L16.4951 16.414C16.5254 16.6772 16.4147 16.9369 16.2037 17.0972C15.9927 17.2575 15.7128 17.2946 15.4674 17.1947L11.2797 15.4913C10.5273 15.6864 9.78118 15.75 9 15.75C4.15905 15.75 0 12.5483 0 8.25ZM9 2.25C4.69095 2.25 1.5 5.04835 1.5 8.25C1.5 11.4517 4.69095 14.25 9 14.25C9.77869 14.25 10.451 14.1792 11.1095 13.9816C11.2734 13.9325 11.4491 13.9408 11.6076 14.0053L14.8598 15.3282L14.5549 12.686C14.5287 12.4585 14.6078 12.2316 14.7697 12.0697C15.8609 10.9785 16.5 9.66018 16.5 8.25C16.5 5.04835 13.3091 2.25 9 2.25Z"
		></path>
	</svg>
);
export const StarEmptyIcon = (
	<svg
		xmlns="http://www.w3.org/2000/svg"
		xmlnsXlink="http://www.w3.org/1999/xlink"
		width="64"
		height="64"
		viewBox="0 0 80 80"
	>
		<defs>
			<path
				id="ce48504d-4377-4585-a7e5-b743f9f5e706-a"
				d="M43.716 5.432l2.859 10.239a7.29 7.29 0 004.228 4.796l9.682 3.99c5.349 2.204 6.112 9.605 1.327 12.87l-8.661 5.913a7.327 7.327 0 00-3.158 5.57l-.708 10.613c-.39 5.86-7.057 8.872-11.592 5.235l-8.212-6.585a7.085 7.085 0 00-6.18-1.354l-10.12 2.57c-5.59 1.418-10.473-4.122-8.492-9.635l3.585-9.982a7.441 7.441 0 00-.66-6.407L2.067 24.24c-3.064-4.985.584-11.419 6.344-11.189l10.428.415a7.106 7.106 0 005.772-2.606l6.692-8.147C35-1.787 42.136-.223 43.716 5.433z"
			></path>
			<path
				id="ce48504d-4377-4585-a7e5-b743f9f5e706-c"
				d="M28.34 5.431l2.86 10.24c.608 2.176 2.168 3.947 4.227 4.796l9.683 3.99c5.348 2.204 6.111 9.604 1.328 12.87l-8.662 5.913a7.326 7.326 0 00-3.158 5.57l-.709 10.613c-.026.39-.08.766-.159 1.13C12.394 55.609-1.723 35.19.251 13.338l3.213.127a7.1 7.1 0 005.77-2.606l6.694-8.147c3.696-4.499 10.834-2.935 12.412 2.72z"
			></path>
			<path
				id="ce48504d-4377-4585-a7e5-b743f9f5e706-e"
				d="M29.628 11.393c.658-3.136 3.67-5.132 6.73-4.458 3.06.673 5.007 3.761 4.35 6.897-.657 3.135-3.67 5.132-6.73 4.458-3.06-.674-5.007-3.761-4.35-6.897zM7.276.53c3.059.674 5.006 3.762 4.349 6.898-.657 3.135-3.67 5.132-6.729 4.457-3.06-.674-5.007-3.762-4.35-6.897.657-3.136 3.67-5.131 6.73-4.458z"
			></path>
			<path
				id="ce48504d-4377-4585-a7e5-b743f9f5e706-g"
				d="M.75 8.56c.165-.784.919-1.283 1.683-1.114l3.957.87c.765.169 1.252.942 1.087 1.725-.164.784-.917 1.283-1.682 1.114l-.499-.11c-.422 5.407 3.124 10.446 8.436 11.616 5.311 1.17 10.564-1.932 12.346-7.04l-.5-.11c-.764-.168-1.251-.94-1.086-1.724.163-.784.917-1.283 1.682-1.114l3.957.871c.765.169 1.252.94 1.087 1.724-.164.784-.918 1.283-1.682 1.114l-.666-.146c-2.126 6.674-8.892 10.77-15.734 9.264-6.84-1.506-11.369-8.09-10.633-15.069l-.665-.147C1.074 10.116.587 9.344.75 8.56zm21.025-4.917c.164-.783.917-1.282 1.683-1.114l.276.06c.765.17 1.252.941 1.087 1.725-.164.784-.917 1.283-1.682 1.114l-.277-.06c-.764-.169-1.251-.94-1.087-1.725zM12.655.15l.277.061c.765.168 1.252.941 1.087 1.724-.164.785-.917 1.284-1.682 1.115l-.278-.06c-.764-.17-1.25-.941-1.087-1.725.165-.785.918-1.283 1.683-1.115z"
			></path>
		</defs>
		<g fill="none" fillRule="evenodd" opacity="0.997">
			<g transform="rotate(-12 67.32 3.796)">
				<mask id="ce48504d-4377-4585-a7e5-b743f9f5e706-b" fill="#fff">
					<use xlinkHref="#ce48504d-4377-4585-a7e5-b743f9f5e706-a"></use>
				</mask>
				<path
					fill="#C7C7C7"
					d="M-6.732-7.793h79.409v81.922H-6.732z"
					mask="url(#ce48504d-4377-4585-a7e5-b743f9f5e706-b)"
				></path>
			</g>
			<g transform="rotate(-12 59.633 -69.346)">
				<mask id="ce48504d-4377-4585-a7e5-b743f9f5e706-d" fill="#fff">
					<use xlinkHref="#ce48504d-4377-4585-a7e5-b743f9f5e706-c"></use>
				</mask>
				<path
					fill="#E0E0E0"
					d="M-7.62-7.793h64.921v76.236H-7.62z"
					mask="url(#ce48504d-4377-4585-a7e5-b743f9f5e706-d)"
				></path>
			</g>
			<g transform="rotate(-12 159.533 -57.659)">
				<mask id="ce48504d-4377-4585-a7e5-b743f9f5e706-f" fill="#fff">
					<use xlinkHref="#ce48504d-4377-4585-a7e5-b743f9f5e706-e"></use>
				</mask>
				<path
					fill="#C7C7C7"
					d="M-7.269-7.49h55.792v33.801H-7.269z"
					mask="url(#ce48504d-4377-4585-a7e5-b743f9f5e706-f)"
				></path>
			</g>
			<g transform="rotate(-12 162.44 -80.152)">
				<mask id="ce48504d-4377-4585-a7e5-b743f9f5e706-h" fill="#fff">
					<use xlinkHref="#ce48504d-4377-4585-a7e5-b743f9f5e706-g"></use>
				</mask>
				<path
					fill="#000"
					fillOpacity="0.54"
					d="M-6.969-7.773h45.907v41.475H-6.969z"
					mask="url(#ce48504d-4377-4585-a7e5-b743f9f5e706-h)"
				></path>
			</g>
		</g>
	</svg>
);

export const thumbnailImage = require("./images/thumbnail.png");
export const ogImage = require("./images/thumbnail.png");
