...
import { filterNotFound, ogImage, thumbnailImage } from "@/assets"
import {
  BoxGridLoading,
  Breadcrumb,
  CategoryLoading,
  ...
  return (
    <>
      <Seo
        ...
        url={`/${category?.parent_category?.[0]?.id}`}
...
export const getStaticProps: GetStaticProps = async (context: GetStaticPropsContext) => {
  ...
  const category = res?.result?.data?.parent_category?.[0]
  ...
  return {
    props: {
      ...
      openGraphData: [
        {
          property: "og:image",
          content: category?.icon ? toImageUrl(category.icon) : ogImage,
          key: "ogimage",
        },
        {
        ...
        {
          property: "og:url",
          content: `${DOMAIN_URL}/category/${category?.id}`,
          key: "ogurl",
        },
        {
